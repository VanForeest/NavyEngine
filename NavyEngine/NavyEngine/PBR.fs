...
vec3 ComputeDirecLight(DirecLight dilight, vec3 N, vec3 V, vec3 F0, vec3 worldpos, float rough, float metalic){
...
vec3 ComputePointLight(PointLight polight, vec3 N, vec3 V, vec3 F0, vec3 worldpos, float rough, float metalic){
...
void main(){
    ...
    Lo += ComputeDirecLight(dirlight, N, V, F0, WorldPos, roughness, metallic);

    for(int i = 0; i < 4; ++i){
        // calcula per-light radiance
        Lo += ComputePointLight(polight[i], N, V, F0, WorldPos, roughness, metallic);
    }   
